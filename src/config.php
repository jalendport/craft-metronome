 ...
 * Like any Craft plugin config file, it's multi-environment aware — nest the
 * `schedule` key under environment keys (`'*'`, `'production'`, …) to vary
 * the schedule per environment.
 *
...
return [
    'schedule' => static function(Schedule $schedule): void {
        // Every task starts with one of the four registration methods:
        //
        //   A Craft console command (extra arguments as the second array):
        //   $schedule->command('resave/entries', ['--section=news'])->hourly();
        //
        //   A raw shell command:
        //   $schedule->exec('/usr/bin/backup.sh')->dailyAt('02:30');
        //
        //   A PHP callable — anything it echoes is captured as its output:
        //   $schedule->call(function() {
        //       // ...
        //   })->everyFiveMinutes();
        //
        //   A queue job — pass a job instance or a job class name:
        //   $schedule->job(\my\plugin\jobs\SyncJob::class)->hourly();

        // …then chains fluent methods to shape when and how it runs:
        //
        //   Frequencies:
        ...
        $schedule->command('utils/update-search-indexes')->daily();
    },
];
